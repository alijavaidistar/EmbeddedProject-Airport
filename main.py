...
class AirportSchedulerApp:
    ...
    def show_scheduler_buttons(self):
        ...
        algo_frame = tk.Frame(self.content_frame, bg=styles.COLOR_BG)
        algo_frame.pack(pady=10)

        tk.Label(algo_frame, text="Select Algorithm:", font=styles.LABEL_FONT, bg=styles.COLOR_BG).pack(side=tk.LEFT)
        self.algo_var = tk.StringVar(value="FCFS")
        algo_menu = ttk.Combobox(algo_frame, textvariable=self.algo_var, values=["FCFS", "Optimized", "EDF", "Round Robin", "Priority Preemptive"])
        algo_menu.pack(side=tk.LEFT, padx=10)

        tk.Button(algo_frame, text="Run Scheduler", font=styles.BUTTON_FONT, command=self.run_selected_scheduler).pack(side=tk.LEFT, padx=20)
        ...
        tk.Label(dropdown_frame, text="Number of Runways:", font=styles.LABEL_FONT, bg=styles.COLOR_BG).pack(
            side=tk.LEFT)
        self.runway_count_var = tk.StringVar()
